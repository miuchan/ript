...
| 12 (executable elementary column refinements) | A typed inductive refinement calculus inserts/deletes forward identity columns, expands/contracts forward composite columns, lifts moves beneath arbitrary common prefixes, and composes transitively; a computable signed width change exactly represents target-minus-source length; every generator has an exact quotient interpretation, both inverse round trips, and prefix-stable semantic cancellation | PROVED |
...
| 12 (global cost-exact complete-Segal/Rezk equivalence) | Extend the elementary forward-column refinements to a common-refinement quotient covering competing expansions and marked reverse columns, prove reduced-hammock moves and their homotopical invariance (or compare them to another accepted derived mapping-space construction), connect that comparison to a standard weak-equivalence interface, and finish the standard Dwyer--Kan/Rezk weak-equivalence and completeness theorem | OPEN_RESEARCH |
