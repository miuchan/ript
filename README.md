...
two components can now be combined: every separable mixed-coordinate
...
are not definitionally componentwise pairs. Beyond that closure, every
...
arrows use a chosen inverse equivalence. Its all-object identity comparison,
...
| `CategoryTheory.Bicategory.mateEquiv_unit` | The source unit followed by a mate and the inverse target unit gives the canonical unitor comparison. |
...
| `CategoryTheory.Pseudofunctor.StrongTrans.hom_comp_inverseNaturalityIso_unit` | A forward constraint followed by its mate-derived inverse transports across the inverse unit to the canonical identity constraint. |
...
| `Ript.Examples.TwoDimensionalWalkingLocalization.generalLiftMapCompForward_naturality_right` | The full forward compositor, including every equality transport, is natural in retained-coordinate 2-cells on its right factor. |
| `Ript.Examples.TwoDimensionalWalkingLocalization.generalLiftMapCompForward_naturality_left` | The full forward compositor, including every equality transport, is natural in retained-coordinate 2-cells on its left factor. |
...
| `Ript.Examples.TwoDimensionalWalkingLocalization.generalLiftMapCompInverseRetained_naturality_right` | The complete inverse-then-retained compositor is natural in retained-coordinate 2-cells on its retained right factor. |
| `Ript.Examples.TwoDimensionalWalkingLocalization.generalLiftMapCompInverseRetained_naturality_left` | The complete inverse-then-retained compositor is natural in retained-coordinate 2-cells carried by its inverse left factor. |
...
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransInverseNaturality_naturality` | Constraints on freely adjoined inverse arrows are natural in every retained-coordinate 2-morphism. |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransEndpointNaturality_inverse_of_not_le` | A genuinely reverse endpoint-normalized constraint is exactly the inverse-mate constraint. |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransEndpointNaturality_inclusion` | On every arrow in the inclusion image, endpoint normalization recovers the original source constraint. |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransEndpointNaturality_naturality` | Endpoint normalization yields a 2-cell-natural constraint for every target arrow, covering both forward and inverse branches. |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransEndpointNaturality_iso` | Endpoint-normalized constraints transport canonically across every target 2-isomorphism. |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransEndpointNaturality_inverseComposite` | The constraint obtained by composing the inverse-generator mate with an arbitrary retained constraint is exactly endpoint normalization on the raw composite. |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransEndpointNaturality_comp_inclusion` | Endpoint-normalized constraints satisfy composition coherence for every pair of arrows in the inclusion image. |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransEndpointNaturality_id_eq` | The endpoint-normalized identity constraint equals the canonical strong-transformation identity constraint. |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransForwardIdentityNaturality_transport` | Transporting source naturality at the canonical forward identity yields the target identity constraint. |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality` | Endpoint normalization, with a canonical identity branch, selects a candidate strong-naturality isomorphism for every target 1-morphism. |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_id` | The public all-arrow candidate satisfies the full strong-transformation identity coherence law. |
...
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransGeneratorCancellation_unit` | Forward-generator then inverse-generator naturality transports across the inverse generator unit to the canonical identity constraint. |
...
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_compIso_generator_inverseGenerator` | Inverse-unit transport injectivity identifies the opposite composed public cancellation candidate with the public raw-composite constraint. |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_comp_generator_inverseGenerator` | The public composition coherence law holds for a strict forward generator followed by its inverse. |
...
| 12, higher-localization specification | Adjoint-equivalence mark inversion, pseudofunctor precomposition, identity and walking-arrow base constructions, and a non-locally-discrete parameterized construction with retained-, localized-, separable mixed-coordinate, and replete-equivalence-closure lift families, local precomposition equivalences, and arbitrary marking-inverting target `PrelaxFunctor` actions | **PROVED** |
...
- [x] Full 2-dimensional localization predicate, identity and walking-arrow base constructions, endpoint-normal-form/thin/codiscrete classification of the completed walking coordinate, a non-locally-discrete inverse-adjoining slice with retained-, localized-, separable mixed-coordinate, and replete-equivalence-closure lift families, local precomposition equivalences, and an arbitrary marking-inverting target `PrelaxFunctor` action with all-object identity and all-arrow composition comparisons; pseudofunctor coherence and the resulting nonseparable factorization remain open
