...
<!-- Keep this wide matrix as explicit HTML: the repository quality gate
checks the 10-cell schema so a malformed Markdown separator cannot silently
degrade into the raw pipe text shown by some renderers. -->
<table>
  <thead>
    <tr>
      <th>Model</th>
...
| Rezk classifying diagram | Outer simplicial category of composable interface strings, followed levelwise by the ordinary nerve | Every vertical level is a groupoid nerve, Kan, strict Segal, quasicategorical, and 2-coskeletal; every horizontal row is naturally an ordinary nerve and hence strict Segal; outer degree `n` is isomorphic to `Map(Δ[n], N(M.Object))`; every transported boundary restriction to `Map(∂Δ[n], N(M.Object))` is a fibration; the actual completeness map is the nerve of a category equivalence | Semantic proof layer; the actual outer spine maps are equivalences in every bidegree; natural matching-limit identification, full Reedy packaging, complete-Segal packaging, and localization remain open |
...
outer degree `n` with `Map(Δ[n], N(M.Object))`. The pushout-product theorem then
proves that restriction to `Map(∂Δ[n], N(M.Object))` is a fibration.
...
Natural identification of the boundary object with the abstract Reedy
matching limit, full Reedy fibrancy packaging, complete-Segal packaging, and
localization remain open. These layers do
