...
| 12 (aligned multi-column hammock fragment) | Endpoint-safe aligned cells decompose equal-shape linear rows into one raw atomic 2-cell per horizontal column; source/target lengths and explicit widths agree, horizontal append is executable and adds widths, quotient interpretation preserves componentwise identities and vertical composition by whiskering functoriality and interchange, and every arbitrary-height aligned grid reconstructs a genuine linear-hammock simplex with exact rows and interpreted edges | PROVED |
| 12 (executable elementary column refinements) | A typed inductive refinement calculus inserts/deletes forward identity columns, expands/contracts forward composite columns, inserts/deletes marked unit and counit pairs, lifts moves beneath arbitrary common prefixes, and composes transitively; a computable signed width change exactly represents target-minus-source length; every generator family has an exact quotient interpretation, both inverse round trips, and prefix-stable semantic cancellation | PROVED |
...
| 12 (global cost-exact complete-Segal/Rezk equivalence) | Replace the zero-truncated thin refinement nerve by a non-thin refinement-path/hammock mapping category retaining competing moves, prove critical-pair coherence and reduced-hammock homotopical invariance (or compare it to another accepted derived mapping-space construction), connect that comparison to a standard weak-equivalence interface, and finish the standard Dwyer--Kan/Rezk weak-equivalence and completeness theorem | OPEN_RESEARCH |
...
- The constructive replacement has now started at the actual full-bicategory
  ...
  outer bridge now also has exact two-arrow vertices, all three faces and both
  identity degeneracies. Relative comparison preserves those vertices, and
  the three horizontal pair vertices of every degree-two local prism glue to
  them; target middle faces decode from actual mapped local composite vertices.
  Arbitrary outer strings now also have canonical vertices with exact
  restriction along every simplex map, and comparison preserves them; every
  source vertex of every all-degree local prism decodes to a source pair with
  ...
  inserted into the relative outer maximal core. The source-defined relative-
  zigzag word/quotient-2-cell nerve is now explicitly compared with the actual
  target local nerve for every model pair: the comparison is a categorical
  nerve equivalence with a displayed simplicial homotopy inverse, and the
  existing local map factors through it strictly in every degree. The outer
  homotopy functor is also essentially surjective. The underlying quotient
  ...
  object and mapping conditions are now combined in one audited
