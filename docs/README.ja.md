...
加えて、別に指定した有限実スペクトルの正確な有理 Gibbs 確率を必要十分条件で分類します。任意の
参照ミクロ状態を選ぶと、それが存在するのは全ての相対 Boltzmann 因子が正の有理数である場合に
限ります。正有理重み `(2, 1)` と `(1, 2, 3)` は実行可能な `(2/3, 1/3)` と
`(1/6, 1/3, 1/2)` を生成し、比が `sqrt 2` の二準位スペクトルには正確な有理 Gibbs 分布がない
ことも厳密に証明済みです。同温の独立系に対する重み・確率の因数分解、分配関数の乗法性、エネルギー・エントロピー・自由エネルギーの
...
分解し、相関補正 Landauer 境界を証明します。正確な有限近似消去も扱います。有理数
...
逆表現は全台再構成定理で証明済みです。非空の隠れ状態を持つ任意の確率的実験に対する逆定理も
Lean カーネルで証明済みです。すべての正確な garbling は決定論的後処理の有理 simplex で
表現され、有理凸包への反射、有理厳密分離完全性、および分離子と健全な有限意思決定分離証明書
の同値も証明済みです。カーネル検証済みの空状態反例は非空仮定が必要であることを示します。
一般の可測因果モデルは研究課題です。一方、明示的な有限熱浴補助プロトコルは実装済みです。
...
実際に逆射を追加する最初の縦断面もコンパイル済みです。walking arrow は局所離散な始域では同値で
ありませんが、Mathlib の自由 groupoid は明示的な逆射を追加し、左右の逆射則、通常の localization
普遍性、誘導された双圏標識反転を満たします。さらに型と関数の一対象双圏との積を取るパラメータ化版は、
標的が局所離散でないこと、全始域 2-射上の忠実性、walking 座標の局所化後も Boolean discard が
...
同値になりました。始域の modification 成分は自由に追加された逆射を越えて持ち上がり、始域の strong
transformation は標的の対象成分と全標的 1-射上の標準自然性同型を定めます。mate 恒等式が二つの生成射
消去順序を処理し、結合子に沿う transport と因子同型に対する不変性が、任意の保持座標を伴う二つの端点変更
消去順序まで結果を拡張します。walking 補完の thin 性により、残るすべての標的合成はこれらの証明済みの
場合へ帰着します。得られたデータは真の標的 strong transformation として正式に組み立てられ、その制限は
可逆 modification により元の始域 strong transformation と同型になります。既存の充満忠実性と合わせて、
局所本質的全射性と局所同値が得られます。
...
標準前向き射は始域作用を再利用し、真に逆向きの射は選択した逆同値を用います。全対象の恒等比較、8 種の端点標準形合成比較、および全ての合成可能な標的射対を覆う一つの比較はコンパイル済みです。全射比較は定義的に端点形へ帰着し、8 本の分岐帰着等式も証明済みです。完全な前向き—前向き compositor と 2 つの混合 compositor（逆射後保持、保持後逆射）は、どちらの保持座標変数についても自然であることが証明済みです。これにはすべての等式 transport、associator、始域 compositor、mate から導かれる逆向き sliding が含まれます。残る 5 分岐の自然性は後続の coherence 作業です。さらに、4 本の端点 hom-functor 帰着定理と一様な端点 2-射コンストラクタが、残る coherence 証明に必要な前向き／逆向き作用を明示します。残る作業は pseudofunctor coherence を証明し、
...
同じ資源情報には、同値性が証明された第二の表現もあります。コスト関数は
`cost(f) ≤ r` によって入れ子の予算層を生成し、恒等射・直列合成・利用可能な場合のテンソルは
それらの層を保ちます。逆に、各プロセスの到達可能な最小許容予算を明示する
`AttainedHomFiltration` から劣加法的コストを再構成できます。二つの往復は厳密であり、
`costToFiltration_toCost` は元のコストを、
...
隠れ状態台が**非空**である任意の有限確率的実験について、
`FiniteBlackwellShermanStein` は証明済みです。すべての有限行動台、正確な事前分布、正確な
損失に対する普遍的リスク順序から、正確な garbling が従います。
...
得られます。幾何学的な橋もカーネル検証済みです。有限実凸包内の有理点を有理凸包へ反射し、実
Hahn--Banach 分離で厳密な実線形汎関数を得て、有理係数ベクトルの稠密性により有限個の厳密不等式
を保存します。これにより有理厳密分離完全性と完全な確率的逆定理が証明されます。真に確率的な
Boolean 例は `1/4 < 1/2` を実行します。この古典的な命題レベルの存在証明は、線形計画双対公理を
仮定せず、最適化器の抽出も主張しません。
...
ままで、対数・積分・非計算性は解析的意味論層だけに留まります。

`FiniteGibbsData` は実エネルギー `E`、正の逆温度 `β`、Boltzmann 重み、有限分配関数を追加します。
各重みと分配関数の正値性、Gibbs 確率の正規化、その対数公式を証明済みです。
...
証明し、実現証明書を構成します。これは全台を持つ正確な分布のエネルギー表示の存在定理です。
別の正確な定理は、任意の参照ミクロ状態について、正規化 Gibbs 確率が有理数であることと、全ての
`exp(-β(E(x) - E(reference)))` が正の有理数であることを同値にし、エネルギー零点に依存しない
完全な分類を与えます。明示的な正有理重みからの構成は二準位・三準位の正確な分布を実行でき、
相対因子 `sqrt 2` の二準位例は厳密な反例です。ただし任意の実指数式の等式を判定する一般手続きは
主張しません。
...
`F(γ)`、自由エネルギー差は積状態上で加法的です。

`WorkAssistedTransition` は、系・目標・電池の正確な端点状態、共通逆温度、Gibbs-preserving
合同チャネル、初期積状態から最終積状態への正確な発展式を明示的に保持します。Ript は系の超過
自由エネルギー増分が電池の超過自由エネルギー減少以下であることを証明します。電池の初期・最終
...
完全相関公平 Boolean 対では `I = log 2`、相関自由エネルギーは `log 2 / β` です。

`Ript.Examples.ApproximateErasure` は正確な有理誤り率
`0 ≤ ε ≤ 1/2` に目標を精密化します。`false` の質量は `1 - ε`、誤り値 `true` の質量は
`ε` であり、

```text
S(目標 ε) = binEntropy ε
F(目標 ε) - F(平衡) = (log 2 - binEntropy ε) / β
```
...
`BathAssistedTransition` は系、熱浴、電池の正確な端点を別々に記録します。一般定理は、系の自由エネルギー増加が熱浴と電池の自由エネルギー減少の和以下であることを証明します。熱浴が正確に戻ると熱浴項は消えますが、電池損失を機械的仕事と呼ぶにはさらにエントロピー中性が必要です。

`Ript.Examples.ExplicitBathErasure` は存在と等号達成の有限な証人を与えます。上の決定的置換は `(公平, 公平, 消去済み)` を `(消去済み, 公平, 公平)` へ正確に写し、一様 Gibbs 状態を保ち、熱浴を戻し、系の増加と電池の減少がともに `log 2 / β` であることを証明します。電池エントロピーは `0` から `log 2` に変化するため、これは情報電池プロトコルであり、機械的仕事プロトコルではありません。

`Ript.Examples.ExactWorkErasure` は熱浴を使わない相補的な機械的仕事証人です。Boolean 仕事電池の Gibbs 重みは `2/3` と `1/3` で、任意の正逆温度において厳密なエネルギー差 `E(high)-E(low)=log 2 / β` を持ちます。完全に実行可能な正確有理チャネルが `(公平なメモリ, 純粋高電池)` を `(消去済みメモリ, 純粋低電池)` へ写し、同時平衡状態を保存します。両端の Shannon エントロピーは厳密にゼロで、電池の平均エネルギー減少とメモリの自由エネルギー増加はいずれも `log 2 / β` です。Lean は非縮退性、エントロピー中性、機械的 Landauer 等号を証明します。

`Ript.Examples.ExactWorkCycle` は対応する逆充電段階を与えます。その Gibbs-preserving チャネルは `(消去済みメモリ, 純粋低電池)` を `(公平なメモリ, 純粋高電池)` へ正確に写し、メモリのランダム化が解放する `log 2 / β` の自由エネルギーで同量の電池充電を支払います。二段階プロトコルの正確な軌跡は `公平/高 → 消去済み/低 → 公平/高` です。各段階は符号付き Landauer 等号を達成し、系自由エネルギー変化と電池エネルギー変化はいずれも全循環で厳密にゼロへ合計されます。したがってこれは閉じた蓄仕事循環であり、正味仕事源ではありません。上の正確な有理性分類はこの電池と任意の有限実スペクトルに適用されます。任意の実指数等式の一般的なアルゴリズム判定だけが実行可能層の外に残ります。
...
証明済みの帰結により、`Quasicategory` インスタンスと
`SimplicialObject.IsCoskeletal M.InterfaceNerve 2` の両方が得られ、高次 simplex は
2-truncation を超える追加データを持ちません。

低次元の解釈は単なる類似ではなく、正確な同値です。
...
合成可能な二つの内部同一性から明示的な 2-simplex を構成します。その第 2 面と第 0 面は入力辺、
中間面は内部合成です。始域が groupoid なので各辺には逆があり、辺とその逆が張る 2-simplex の
合成面は退化した reflexivity 辺になります。

この nerve は元の 1-圏論的 homotopy 情報を正確に保ちます。
...
Boolean 例では、tensor 対称辺を元の内部 path と構造同値の双方へ復号します。正方向と逆方向の辺は
...
外側比較も全次数で一様に形式化されています。垂直次数 `k` を固定し、`Fin (n + 1)` と
`Fin (k + 1)` を入れ替えることで、水平 simplicial set 全体を
`ComposableArrows M.Object k` の通常の nerve と自然に同一視します。
...
| `Ript.Resource.filtrationToCost_tensor` | テンソル互換フィルトレーションは並列劣加法的コストを再構成します。 |
...
| `Ript.ForMathlib.RationalConvexHull.mem_convexHull_of_ratCastVector_mem_convexHull` | 有限実凸包内の有理メンバーシップは有理凸包へ反射します。 |
| `Ript.ForMathlib.RationalConvexHull.exists_rational_strictSeparator_of_not_mem_convexHull` | 有限有理凸包外の各有理点には正確な有理厳密分離子があります。 |
| `Ript.Models.Decision.RationalSeparation.channelVector_mem_convexHull_iff` | Blackwell 支配は、目標チャネルベクトルが決定論的後処理の凸包に属することと同値です。 |
| `Ript.Models.Decision.RationalSeparation.rationalSeparationComplete` | Garbling を持たない各有限実験対には正確な有理厳密分離子があります。 |
...
| `Ript.Models.Decision.RationalSeparation.blackwellShermanSteinConverse` | 非空の隠れ状態では、各実験対の普遍的有限意思決定順序から正確な garbling が従います。 |
| `Ript.Models.Decision.RationalSeparation.finiteBlackwellShermanStein` | 宇宙多相な完全有限確率的 Blackwell--Sherman--Stein 逆定理が証明済みです。 |
...
| `Ript.Models.Thermal.FiniteGibbsData.hasRationalProbabilities_iff_hasRationalBoltzmannRatiosAt` | 別に指定した有限実スペクトルが正確な有理 Gibbs 確率を持つことは、参照状態への全 Boltzmann 比が正の有理数であることと同値です。 |
| `Ript.Models.Thermal.FiniteGibbsData.ofPositiveRationalWeights_probability` | 正有理 Boltzmann 重みは実行可能な正規化有理平衡を正確に生成します。 |
| `Ript.Examples.RationalGibbsSpectra.irrationalTwoLevelSpectrum_not_hasRationalProbabilities` | Boltzmann 比が `sqrt 2` の二準位スペクトルに正確な有理 Gibbs 分布は存在しません。 |
...
| `Ript.Models.Thermal.GibbsThermalObject.meanEnergy_pure` | 純粋な有限状態の平均エネルギーは、その支持ミクロ状態のエネルギーです。 |
| `Ript.Models.Thermal.GibbsThermalObject.entropy_pure` | すべての純粋有限状態の Shannon エントロピーはゼロです。 |
| `Ript.Examples.ExactWorkErasure.exactWorkErasureChannel_erases` | 実行可能な二ビットチャネルは、仕事電池を放電しながらメモリを正確に消去します。 |
| `Ript.Examples.ExactWorkErasure.workBattery_low_lt_high` | 偏った二準位電池は、すべての正逆温度で厳密に非縮退です。 |
| `Ript.Examples.ExactWorkErasure.exactWorkErasure_batteryEntropy_neutral` | 純粋高・純粋低電池端点のエントロピーは厳密に等しいです。 |
| `Ript.Examples.ExactWorkErasure.exactWorkErasure_saturates_landauer_work` | 電池の供給エネルギーとメモリ自由エネルギー増加はいずれも `log 2 / β` です。 |
| `Ript.Models.Thermal.FiniteClosedProtocol.trace_twoSteps` | 認証済みの往復遷移対は正確な三状態閉軌跡を持ちます。 |
| `Ript.Examples.ExactWorkCycle.exactWorkRechargeChannel_recharges` | 実行可能な充電チャネルは消去済み/低を公平/高へ正確に写します。 |
| `Ript.Examples.ExactWorkCycle.exactWorkRecharge_saturates_landauer_work` | メモリ自由エネルギーの解放が電池エネルギー増加を正確に支払います。 |
| `Ript.Examples.ExactWorkCycle.exactWorkCycle_trace` | 閉循環は `公平/高 → 消去済み/低 → 公平/高` をたどります。 |
| `Ript.Examples.ExactWorkCycle.exactWorkCycle_returns` | 消去後の充電はメモリ–電池の完全状態を正確に戻します。 |
| `Ript.Examples.ExactWorkCycle.exactWorkCycle_batteryEnergy_balanced` | 放電・充電の符号付き電池エネルギー変化の和はゼロです。 |
| `Ript.Examples.ExactWorkCycle.exactWorkCycle_systemFreeEnergy_balanced` | メモリの符号付き自由エネルギー変化の和はゼロです。 |
...
| `Ript.Examples.SimpleThermalModel.approximateErasureCost_antitone` | 正確な近似消去費用は有理誤り区間 `[0, 1/2]` で単調非増加です。 |
| `Ript.Examples.SimpleThermalModel.approximateErasedBit_freeEnergyGap` | 誤り `ε` の目標は正確な超過自由エネルギー `(log 2 - binEntropy ε) / β` を持ちます。 |
| `Ript.Examples.SimpleThermalModel.thermalBit_approximate_erasure_landauer_work_bound` | 積端点近似消去は二項エントロピー欠損に対応する仕事を必要とします。 |
| `Ript.Examples.SimpleThermalModel.thermalBit_correlated_approximate_erasure_landauer_work_bound` | 相関近似消去は相関自由エネルギー増分も支払います。 |
...
| `CategoryTheory.LocallyDiscrete.equivalenceOfIsIso` | 通常の圏同型は対応する局所離散双圏で随伴同値を誘導します。 |
| `CategoryTheory.Bicategory.MorphismProperty.locallyDiscrete_isInvertedBy` | 通常の反転は誘導 pseudofunctor により双圏的な随伴同値反転へ移送されます。 |
| `CategoryTheory.Bicategory.MorphismProperty.IsInvertedBy.of_equivalence` | 始域 pseudofunctor を随伴同値なものへ置き換えても標識反転は保たれます。 |
| `CategoryTheory.Pseudofunctor.FactorsThrough.trans` | pseudofunctor を通る因子化は始域 pseudofunctor の随伴同値に沿って移送できます。 |
| `CategoryTheory.Bicategory.mateEquiv_sliding` | 左随伴平方間の可換平方は、mate により対応する右随伴平方間の可換平方になります。 |
| `CategoryTheory.Bicategory.mateEquiv_counit` | mate の後に標的 counit を合成することは、元の平方の後に始域 counit を合成することと等しく、双圏 coherence も明示されます。 |
| `CategoryTheory.Bicategory.mateEquiv_unit` | 始域 unit、mate、標的 unit の逆を合成すると標準 unitor 比較になります。 |
| `CategoryTheory.Pseudofunctor.StrongTrans.inverseNaturalityIso_comp_hom_counit` | mate から得た逆制約と前向き制約の合成は、counit に沿って transport すると標準恒等制約になります。 |
| `CategoryTheory.Pseudofunctor.StrongTrans.hom_comp_inverseNaturalityIso_unit` | 前向き制約と mate から得た逆制約の合成は、逆 unit に沿って transport すると標準恒等制約になります。 |
| `CategoryTheory.Pseudofunctor.map_mateEquiv` | pseudofunctor は合成子と単位子の coherence を含めて双圏 mate を保存します。 |
| `CategoryTheory.Pseudofunctor.StrongTrans.inverseNaturalityIso_sliding` | 前向き合成 coherence は随伴同値を越えて mate 由来の逆制約へ滑移します。 |
| `CategoryTheory.Pseudofunctor.StrongTrans.naturalityIsoOfIso_injective` | 固定された可逆な標的 2-射に沿う移送は、strong-naturality 制約候補について単射です。 |
...
| `Ript.Examples.WalkingLocalization.inclusionFunctor_isLocalization` | walking arrow の自由 groupoid への埋め込みは Mathlib の真正な通常 localization です。 |
| `Ript.Examples.WalkingLocalization.inclusion_map_arrow_comp_inverse` | 生成射と新しく追加された逆射の合成は恒等射です。 |
| `Ript.Examples.WalkingLocalization.inverse_comp_inclusion_map_arrow` | 新しく追加された逆射と生成射の合成も恒等射です。 |
| `Ript.Examples.WalkingLocalization.arrow_not_isEquivalence` | walking 生成射は localization 前には双圏同値ではありません。 |
| `Ript.Examples.WalkingLocalization.inclusion_genuinely_adds_inverse` | walking localization はこの真に非可逆な射を随伴同値へ変えます。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.inclusion_inverts` | 積 pseudofunctor は第 2 双圏座標を保持しながら第 1 座標の標識射を反転します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.inclusion_map₂_injective` | パラメータ化 walking localization は全始域 2-射上で忠実です。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.inclusion_map₂_discardTwoCell_not_isIso` | walking 座標の局所化後も Boolean discard は非可逆 2-射です。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.target_not_isLocallyDiscrete` | localization の標的は局所離散双圏でないことが形式証明されています。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.inclusion_adds_inverse_and_retains_noninvertible_twoCell` | 同じコンパイル済み構成が欠けた 1-射の逆を追加し、非可逆 2-射を保持します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.retainedSource_has_factorization` | 任意の標的双圏について、保持座標のみに依存する各 pseudofunctor は localization 標的を通って因子化します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.localizedCoordinateSource_has_factorization` | groupoid に値を取り、局所化される walking 座標のみに依存する各関手は自由 groupoid 標的を通って因子化します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.localizedCoordinateLift_map_inverse` | lift 後の関手は形式的に追加された逆射を元の生成射の像の実際の逆射へ写します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.localizedCoordinate_inverts_factors_and_maps_inverse` | 局所化座標族全体が標識を反転し、因子化し、新しい逆射を正しく解釈します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.separableMixedSource_has_factorization` | 任意の groupoid 値 `K` と保持座標 `H` について、可分離混合 pseudofunctor `K × H` は標的を通って因子化します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.repleteSeparableMixedSource_inverts_and_factors` | 可分離混合始域と随伴同値な各 pseudofunctor は、自動的に標識を反転し標的を通って因子化します。 |
...
| `Ript.Examples.TwoDimensionalWalkingLocalization.generalLiftMapCompForward_naturality_right` | すべての等式 transport を含む完全な前向き compositor は、右因子の保持座標 2-射について自然です。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.generalLiftMapCompForward_naturality_left` | すべての等式 transport を含む完全な前向き compositor は、左因子の保持座標 2-射について自然です。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.generalLiftMapCompInverseRetained` | 標準逆射に保持データを続ける合成比較が構成済みです。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.generalLiftMapCompInverseRetained_naturality_right` | 完全な逆射後保持 compositor は、右の保持因子上の保持座標 2-射について自然です。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.generalLiftMapCompInverseRetained_naturality_left` | 完全な逆射後保持 compositor は、左の逆因子が運ぶ保持座標 2-射について自然です。 |
...
| `Ript.Examples.TwoDimensionalWalkingLocalization.generalLiftMapCompRetainedInverse_naturality_right` | 完全な保持後逆射 compositor は、右の逆因子が運ぶ保持座標 2-射について自然です。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.generalLiftMapCompRetainedInverse_naturality_left` | 完全な保持後逆射 compositor は、左の保持因子上の保持座標 2-射について自然です。 |
...
| `Ript.Examples.TwoDimensionalWalkingLocalization.inclusionLocalPrecompositionFullyFaithful` | 前合成は strong transformation と modification の全局所圏で充満忠実です。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTrans` | 各始域 strong transformation は、すべての恒等・自然性・合成則を満たす真の標的 strong transformation へ延長されます。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransRestrictionIso` | lift された標的 strong transformation の制限は、可逆 modification を介して元の始域 strong transformation を復元します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.inclusion_localPrecomposition_essSurj` | 前合成は各局所圏の対象上で本質的全射です。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.inclusion_localPrecomposition_isEquivalence` | 前合成は strong transformation と modification の全局所圏で同値です。 |
...
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransInverseNaturality_naturality` | 自由に追加された逆向き射の制約は、任意の保持座標 2-射に関して自然です。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransEndpointNaturality_inverse_of_not_le` | 真に逆向きの端点正規化制約は、逆 mate 制約と正確に一致します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransEndpointNaturality_inclusion` | inclusion の像にある任意の射上で、端点正規化は元の始域制約を復元します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransEndpointNaturality_naturality` | 端点正規化は、前向き・逆向きの両分岐を覆う 2-射自然な制約をすべての標的射に与えます。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransEndpointNaturality_iso` | 端点正規化制約は任意の標的 2-同型に沿って標準的に移送されます。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransEndpointNaturality_inverseComposite` | 逆生成射 mate と任意の保持制約の明示的合成は、生の合成における端点正規化制約と正確に一致します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransEndpointNaturality_comp_inclusion` | 端点正規化制約は inclusion の像にある任意の射の組に対して合成 coherence を満たします。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransEndpointNaturality_id_eq` | 厳密な恒等射における端点正規化制約は、標準 strong-transformation 恒等制約と等しいです。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransForwardIdentityNaturality_transport` | inclusion の単位比較に沿って標準前向き恒等射の始域自然性を移送すると、標的恒等制約が得られます。 |
...
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_id` | 公開される全射候補は strong transformation の完全な恒等 coherence を満たします。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_forward` | 各標準前向き射上の公開制約は元の始域制約そのものです。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_generatorInverse` | 厳密な逆向き射上の公開制約は逆生成射 mate と正確に一致します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransGeneratorCancellation_counit` | 逆生成射の後に前向き生成射を合成した自然性は、counit に沿って標準恒等制約へ transport されます。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransGeneratorCancellation_unit` | 前向き生成射の後に逆生成射を合成した自然性は、逆 unit に沿って標準恒等制約へ transport されます。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_compIso_inverseGenerator_generator` | transport の単射性により、合成した公開消去候補は生の合成上の公開制約と一致します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_comp_inverseGenerator_generator` | 厳密な逆生成射の後にその前向き生成射を合成する場合の公開合成 coherence 則を証明します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_compIso_generator_inverseGenerator` | 逆 unit transport の単射性により、反対順序の公開消去候補は生の合成上の公開制約と一致します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_comp_generator_inverseGenerator` | 厳密な前向き生成射の後にその逆射を合成する場合の公開合成 coherence 則を証明します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_iso` | 公開される全射制約は任意の標的 2-同型に沿って移送されます。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_inverseComposite` | 逆生成射/保持座標の生の合成上の公開制約は、明示的な構成子レベルの合成と等しいです。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_naturality` | 公開される全射制約は厳密な恒等分岐を含む任意の標的 2-射に関して自然です。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_comp_inclusion` | 公開制約は inclusion の像にある任意の射の組に対して合成 coherence を満たします。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_comp_forward` | 任意の保持座標を持つ標準的な前向き—前向き合成 coherence が成立します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_generatorRetained_transport` | 前向き生成射に保持制約を続けた合成は、結合された公開前向き制約へ transport されます。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_retainedGenerator_transport` | 保持制約に前向き生成射を続けた合成も、同じ公開前向き制約へ transport されます。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_comp_inverseGenerator_retained` | 逆生成射 mate に任意の保持座標制約を続ける構成子レベルの合成 coherence が成立します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_comp_inverseGenerator_retained_public` | 逆生成射/保持座標の合成則が両方の公開制約を用いて成立します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_inverseGeneratorRetained_transport` | 公開逆生成射/保持合成は標準逆射の公開制約へ transport されます。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransRetainedInverse_sliding` | 保持後逆生成射の候補は標準 mate 比較により逆生成射後保持の候補へ滑移します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.LiftedStrongTransRetainedInverseCompositionCoherence` | 保持後に逆生成射を続ける正確な公開合成命題；次の定理が inhabit します。 |
| `Ript.Examples.TwoDimensionalWalkingLocalization.liftedStrongTransNaturality_comp_retainedInverse` | 全 walking 射について保持後逆生成射の公開合成命題を証明します。 |
...
| `Ript.Univalent.UniverseModel.interfaceNerveComposition_composite` | 合成 2-simplex の中間面は内部 path 合成です。 |
...
| `Ript.Univalent.UniverseModel.interfaceClassifyingDiagramHorizontalRowIso` | 各水平行は通常の圏論的 nerve と自然同型です。 |
| `Ript.Univalent.UniverseModel.interfaceClassifyingDiagramHorizontalStrictSegal` | 各水平行は明示的な strict-Segal 再構成データを持ちます。 |
| `Ript.Univalent.UniverseModel.interfaceClassifyingDiagramOuterSegalEquiv` | 全水平・垂直次数で外側 spine 比較は同値です。 |
...
| 6 | Blackwell 順序、有限意思決定リスク、決定論的および完全有限確率的逆定理、必要な非空状態境界、正確な有理 garbling simplex、有理凸包反射と厳密分離、意思決定証明書、資源予算、タスク相対価値 | **PROVED** |
...
| 正確な有限意思決定層 | `FinStoch` を介して可 | ネイティブ tensor なし | 実行可能 | 順方向リスク順序；決定論的および完全有限確率的逆定理；必要な非空状態境界；正確な有理 garbling simplex；有理凸包反射と厳密分離；有理分離子/意思決定証明書同値 |
...
| 有限熱系 | Gibbs-preserving 圏；有限閉および熱浴補助プロトコル | 積 bifunctor | 正確な状態/チャネル/トレース/周辺、正有理重みの正規化、情報・仕事電池証人は実行可能；実指数等式と Gibbs/KL/自由エネルギー/仕事意味論は非計算的 | 正確な有理性の必要十分分類、正有理重みスペクトル、`sqrt 2` 反例、閉系消去不可能性、Gibbs/KL/自由エネルギー理論、相関分解、有理誤り境界、熱浴別収支と等号達成 |
...
証明済みの観測・介入意味論があります。有理凸包反射、厳密分離、分離子/意思決定証明書同値、
非空の隠れ状態に対する完全な確率的 Blackwell--Sherman--Stein 逆定理も証明済みです。一般可測意思決定問題、異種または可測な因果モデル、
...
[CONJECTURES.md](../CONJECTURES.md) を参照してください。現在、未証明の登録命題はありません。
同じ台帳に、最近解決した有限確率的 Blackwell--Sherman--Stein 逆定理とその証明境界も記録しています。
...
`Ript/Examples/StochasticSeparation.lean` は二つの真に確率的な Boolean 実験を分離します。
精度 `3/4` のノイズ付き観測を直接使うと正確なリスクは `1/4`、独立な公平観測に基づく全規則の
リスクは `1/2` です。格納された証明書は一般健全性定理を使って全確率的 garbling を排除します。
3 個の通常の `#eval decide` 契約が両リスクとその厳密な順序を検査します。
...
Landauer 仕事境界を証明します。一つの `#eval decide` 契約が三つの境界目標の六つの確率質量を
正確に検査します。
...
-- または有限実 Gibbs スペクトルの正確な有理性分類：
import Ript.Models.Thermal.RationalGibbs
-- または正確な有理誤り近似消去とその Landauer 境界：
import Ript.Examples.ApproximateErasure
-- またはエントロピー中性な機械的仕事の正確な等号証人：
...
| [`Ript/Higher/`](../Ript/Higher/) | 資源添字付きモデル双圏、coherence、homotopy category、コスト完全 localization |
...
| [CONJECTURES.md](../CONJECTURES.md) | 未証明命題と最近解決した命題の正式な記録 |
...
12. **熱力学の境界を明示する。** 指定平衡は正確で実行可能な操作データですが、実エネルギー、
    指数、対数、自由エネルギーは非計算的解析層にあります。`GibbsThermalObject` が両者の確率の
    等しさを明示的に認証し、KL/自由エネルギー定理は証明済み Markov-kernel DPI を使います。
    一般の指数重みが有理だとは仮定しません。
...
- [x] 有理凸包反射、有理厳密分離完全性、完全有限確率的 Blackwell--Sherman--Stein 逆定理
...
- [x] 有理 garbling simplex 外の各有理点に対する有理厳密分離完全性を証明し、一般確率的有限 Blackwell--Sherman--Stein 逆定理を完成
...
- [x] 熱浴別 Landauer 収支と、熱浴を正確に戻し情報電池で支払う実行可能な等号達成例
- [x] 非縮退二準位仕事電池によるエントロピー中性な正確消去と `log 2 / β` 等号達成
- [x] 完全状態復帰と符号付き正味変化ゼロを持つ正確な閉仕事電池充電循環
- [x] 別に指定した有限実スペクトルの正確な有理 Gibbs 分類、構成的な正有理重み例、無理数反例
- [ ] 任意の実 Boltzmann 因子等式を判定する一般手続き
...
- [x] 完全な 2 次元 localization 述語、恒等・walking-arrow 基礎構成、walking 補完の端点正規形・thin・codiscrete 分類、保持座標・局所化座標・可分離混合座標・随伴同値 replete 閉包の因子化と局所前合成同値を持つ非局所離散逆射追加スライス、および全対象恒等比較と全射合成比較を持つ任意の標識反転始域 `PrelaxFunctor` 作用；pseudofunctor coherence と最終的な非可分離因子化は未解決
...
細分化により、決定論的有限実験の逆定理も証明済みです。一般の確率的意思決定についても、必要な
非空隠れ状態仮定付きの正確な有限逆定理を証明しました。正確な garbling は決定論的後処理の
有理混合として表され、有理凸包反射と厳密分離完全性はカーネル検証済みで、有理厳密分離子は
健全な意思決定証明書と同値です。ノイズ付き Boolean 証明書は実行可能です。一般可測意思決定
理論はまだ未証明です。
