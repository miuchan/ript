...
標準的な前向き—前向きの場合を含みます。これらのデータを標的 strong transformation にするには、自由に
追加された逆射を含む混合合成の場合だけが残っています。
