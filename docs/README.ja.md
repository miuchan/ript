...
逆表現定理、エントロピー中性な仕事循環は研究課題です。一方、明示的な有限熱浴補助プロトコルは実装済みです。
`((系, 熱浴), 電池) -> ((電池, 熱浴), 系)` という 3 ビット置換は系を正確に消去し、熱浴を完全に戻し、情報電池で `log 2 / β` を支払います。電池エントロピーの変化も証明されているため、これは機械的仕事循環ではありません。
...
`Ript.Examples.ExplicitBathErasure` は存在と等号達成の有限な証人を与えます。上の決定的置換は `(公平, 公平, 消去済み)` を `(消去済み, 公平, 公平)` へ正確に写し、一様 Gibbs 状態を保ち、熱浴を戻し、系の増加と電池の減少がともに `log 2 / β` であることを証明します。電池エントロピーは `0` から `log 2` に変化するため、これは情報電池プロトコルです。エントロピー中性な仕事循環と、別に指定した実スペクトルの有理 Gibbs 重み分類は未解決です。
...
| 8 | 有限平衡系、閉プロトコルの消去不可能性、Gibbs/KL/自由エネルギー理論、相関分解、正確/有理誤り Landauer 境界、熱浴別収支、情報電池による等号達成例 | **PROVED** |
...
| 有限熱系 | Gibbs-preserving 圏；有限閉および熱浴補助プロトコル | 積 bifunctor | 正確な状態/チャネル/トレース/周辺と 3 ビット証人は実行可能；Gibbs/KL/自由エネルギー/仕事意味論は非計算的 | 閉系消去不可能性、Gibbs/KL/自由エネルギー理論、相関分解、有理誤り境界、熱浴別収支、情報電池等号達成 |
...
別に指定した実エネルギースペクトルに対する有理 Gibbs 重みの分類、エントロピー中性な仕事循環、
...
- [ ] エントロピー中性な仕事循環と、別に指定した実エネルギースペクトルの有理 Gibbs 重み分類
...
熱浴別の自由エネルギー/仕事境界と、熱浴を戻して情報電池で等号を達成する実行可能な 3 ビットプロトコルも証明済みです。電池エントロピーが変化するため、これはエントロピー中性な仕事循環ではありません。そのような循環と、別に指定した実スペクトルの有理 Gibbs 重み分類は未実装です。正確な有限
