...
本プロジェクトは、確率・因果性・熱力学・量子理論・高次圏よりも基礎的な層から意図的に
始めています。これらは現時点の機能ではなく研究方向です。現在の Ript は、プロセス合成や
資源会計の意味を暗黙に変えることなく、将来の層を追加するための検証済み土台を提供します。

> [!IMPORTANT]
> Ript は初期段階の研究ソフトウェアです。Stage 1 と Stage 2 は実装済みで Lean の
...
| 1 | 直列資源プロセスの核 | **PROVED** |
| 2 | テンソル、対称性、構造的配線変更、並列資源 | **PROVED** |
| 3 | 実行可能な有限確率モデル | **OPEN RESEARCH** |
...
コピー、破棄、凸性、因果性、熱的構造、確率的意味論、量子チャネル、ユニバレント構造、
高次圏構造は**未実装**です。正式な能力表は [MODEL_MATRIX.md](../MODEL_MATRIX.md)、形式的に
追跡する未解決命題は [CONJECTURES.md](../CONJECTURES.md) を参照してください。現在、登録
された予想はありません。
...
| 具体モデル | `Ript.Models.*` | 有限なコスト 0 関数と明示的計量関数 |
...
現在の主要定理の監査では、必要な箇所に Lean 標準の `propext` と `Quot.sound` のみが現れます。
`Classical.choice`、コンパイラ信頼の回避、穴埋め証明の公理はありません。商への依存は証明意味論
の項モデルに限定され、実行可能構文と有限評価には入りません。
...
| [`Ript/Models/`](../Ript/Models/) | 具体的な有限決定論モデル |
...
7. **実装と構想を区別する。** 確率、因果、熱、量子、高次の層は明確に未解決研究と表示します。
...
いいえ。有限確率、Kleisli、熱、因果、量子モデルはロードマップ項目です。現在の実行可能モデルは
決定論的な有限関数だけです。

### モノイダル層があればコピーや破棄も可能ですか？

いいえ。テンソルと対称性だけから対角射や終対象への射は得られません。コピーと破棄は固有の法則を
持つ明示的能力として導入する必要があります。
