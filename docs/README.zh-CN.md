...
每条 walking 箭头都满足公共复合律。在把这些数据组成目标强变换前，仍需证明两种生成元消去次序。walking 自由群胚本身
...
| 二维 walking localization | 在一个坐标中作自由群胚反转 | 与类型单对象双范畴取积 | 不可计算证明层 | 加入显式缺失逆元，证明端点规范形、thin 性与同 `Fin 2` 上 codiscrete 群胚的等价，保留不可逆 Boolean discard，分解每个保留坐标伪函子、每个取值于群胚的局部化坐标函子、每个可分离混合族 `K × H` 及其完整伴随等价闭包，正确解释形式逆元，预复合局部充满忠实，并为预期强变换提升的每个目标 1-态射重建候选约束；恒等 coherence、全箭头 2-胞自然性、inclusion 像中任意一对箭头的复合 coherence，以及两个逆元/保留坐标次序的公共因子 coherence 都已由 mate 滑移证明；两种消去次序、该闭包之外任意不可分离混合坐标分解和局部本质满射仍开放 |
