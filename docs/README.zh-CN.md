...
自由群胚目标显式分解，而且其提升把形式加入的逆元映为生成箭头像的实际逆元。完整资源过程构造
仍须分解任意混合坐标的反转标记伪函子，并证明局部满性与本质满射；现有桥接仍不是完整的双范畴、Dwyer--Kan、
...
| 12，高阶 localization 规格 | 到伴随等价的标记反转、伪函子预复合、恒等与 walking-arrow 基例，以及具有保留坐标与局部化坐标两类分解及局部忠实性的非局部离散参数化构造 | **PROVED** |
...
| 二维 walking localization | 在一个坐标中作自由群胚反转 | 与类型单对象双范畴取积 | 不可计算证明层 | 加入显式缺失逆元，保留不可逆 Boolean discard，分解每个保留坐标伪函子与每个取值于群胚的局部化坐标函子，正确解释形式逆元，且预复合局部忠实；任意混合坐标分解、局部满性和局部本质满射仍开放 |
...
- [x] 完整二维 localization 谓词、恒等与 walking-arrow 基础构造、具有保留坐标与局部化坐标两类分解和局部忠实预复合的非局部离散逆元加入切片，以及恒等候选障碍；任意混合坐标分解、局部满性和局部本质满射仍开放
