...
| 12，高阶 localization 规格 | 到伴随等价的标记反转、伪函子预复合、恒等与 walking-arrow 基例，以及具有保留坐标、局部化坐标、可分离混合坐标和伴随等价 replete 闭包分解及局部充满忠实性的非局部离散参数化构造 | **PROVED** |
...
| 二维 walking localization | 在一个坐标中作自由群胚反转 | 与类型单对象双范畴取积 | 不可计算证明层 | 加入显式缺失逆元，保留不可逆 Boolean discard，分解每个保留坐标伪函子、每个取值于群胚的局部化坐标函子、每个可分离混合族 `K × H` 及其完整伴随等价闭包，正确解释形式逆元，且预复合局部充满忠实；该闭包之外任意不可分离混合坐标分解和局部本质满射仍开放 |
...
- [x] 完整二维 localization 谓词、恒等与 walking-arrow 基础构造、具有保留坐标、局部化坐标、可分离混合坐标和伴随等价 replete 闭包分解及局部充满忠实预复合的非局部离散逆元加入切片，以及恒等候选障碍；该闭包之外任意不可分离混合坐标分解和局部本质满射仍开放
