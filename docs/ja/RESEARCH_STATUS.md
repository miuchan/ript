...
正しい相対 Rezk 始域もコンパイル済みです。外側 `n` 次はすべての始域列を含み、垂直変換は成分ごとに cost-exact です。実際の局所化関手は、すべての内外単体次数でこれを終域 Rezk core へ自然に写し、表示された矢印頂点上で厳密に計算します。実際の局所 0-単体と任意の 2-セル辺の写像/復号も厳密で、可逆と限らない各 2-セルについて両相対外側端点と対象局所辺が一括されます。残るのは 2 次以上の接着と Dwyer–Kan/Rezk 弱同値です。
